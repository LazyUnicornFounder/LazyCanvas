...
const AuthModal = ({ open, onClose, onSuccess }: AuthModalProps) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative bg-card border border-border rounded-xl shadow-2xl w-full max-w-sm p-6 space-y-5">
        <button
          onClick={onClose}
        ...
        <button
          ...
          className="w-full flex items-center justify-center gap-2 px-4 py-2.5 border border-border rounded-md text-sm font-heading font-medium text-foreground hover:bg-accent transition-colors"
        >
        ...
        <div className="flex items-center gap-3">
          ...
          <span className="text-[10px] text-muted-foreground uppercase tracking-widest font-heading">or</span>
          <div className="flex-1 h-px bg-border" />
        </div>
        ...
        <p className="text-center text-xs text-muted-foreground">
          {mode === "signup" ? (
            <>Already have an account?{" "}
              <button onClick={() => setMode("login")} className="text-foreground hover:underline font-medium">Sign in</button>
            </>
          ) : (
